...
public class BossChargeState : BossBaseState
{
    ...
    public void OnChargeAnimationEnd(BossFSM boss)
    {
        ...
        boss.StartCoroutine(SmoothTransitionToBreathing(boss));
    }
    ...
    private IEnumerator SmoothTransitionToBreathing(BossFSM boss)
    {
        float transitionDuration = 0.5f; // 過渡持續時間
        float elapsed = 0f;
        Vector3 startPos = boss.transform.position;
        // 這裡假設我們不需要改變目標位置，只是讓動作過渡更平滑
        Vector3 endPos = startPos; // 或者你可以指定特定位置

        while (elapsed < transitionDuration)
        {
            boss.transform.position = Vector3.Lerp(startPos, endPos, elapsed / transitionDuration);
            elapsed += Time.deltaTime;
            yield return null;
        }
        boss.transform.position = endPos;
        // 過渡完畢後，轉換到 Breathing 狀態
        boss.TransitionToState(boss.breathingState);
    }
}
